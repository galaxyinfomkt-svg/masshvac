...
export default function Header() {
  ...
  return (
    <>
      {/* Top Bar - like RS Development */}
      <div className={`fixed top-0 left-0 right-0 z-[60] transition-all duration-500 ${scrolled ? "h-0 opacity-0 pointer-events-none" : "h-10 opacity-100"} bg-primary-dark`}>
      ...
      {/* Main Header */}
      <header className={`fixed left-0 right-0 z-50 transition-all duration-300 bg-black shadow-[0_4px_20px_rgba(0,0,0,0.3)] ${scrolled ? "top-0" : "top-10"}`}>
        <nav className="mx-auto max-w-7xl px-4 py-3 flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-1">
            {navLinks.map((link) => (
              <div key={link.href} className="relative" onMouseEnter={() => link.hasDropdown && setServicesOpen(true)} onMouseLeave={() => link.hasDropdown && setServicesOpen(false)}>
                <Link href={link.href} className="flex items-center gap-1 px-4 py-2.5 rounded-lg text-sm font-medium text-white/80 hover:text-accent transition-colors duration-200">
                  {link.label}
                ...
                {link.hasDropdown && (
                  <div className={`absolute top-full left-1/2 -translate-x-1/2 pt-3 transition-all duration-300 ${servicesOpen ? "opacity-100 visible translate-y-0" : "opacity-0 invisible -translate-y-2"}`}>
                    <div className="bg-white rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.2)] p-2 min-w-[320px]">
                      {serviceLinks.map((s) => (
                        <Link key={s.href} href={s.href} className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-surface transition-colors group/item">
                          ...
                          <span className="text-sm font-medium text-gray-700 group-hover/item:text-accent transition-colors">{s.label}</span>
                        </Link>
                      ))}
                      <div className="border-t border-gray-100 mt-1 pt-1">
                        <Link href="/services" className="flex items-center justify-center gap-1 px-4 py-2.5 text-sm font-bold text-accent hover:bg-accent/5 rounded-lg transition-colors">View All Services →</Link>
                      </div>
                    </div>
                  </div>
                )}
          ...
          <div className="hidden lg:flex items-center gap-3">
            <a href="tel:+15083869104" className="flex items-center gap-2 px-6 py-2.5 bg-accent hover:bg-accent-dark text-white font-bold text-sm rounded-lg transition-all duration-300 hover:scale-105">
              <Phone className="w-4 h-4" />(508) 386-9104
            </a>
            <Link href="#contact" className="px-6 py-2.5 border-2 border-white text-white font-bold text-sm rounded-lg transition-all duration-300 hover:bg-white hover:text-black">
              Free Estimate
            </Link>
          </div>
      ...
      <div className={`fixed top-0 right-0 bottom-0 z-[56] w-[85%] max-w-sm bg-dark-900 shadow-2xl transition-transform duration-500 ease-out lg:hidden ${mobileOpen ? "translate-x-0" : "translate-x-full"}`}>
        <div className="flex flex-col h-full">
          ...
          <div className="p-4 border-t border-white/10 space-y-3">
            <a href="tel:+15083869104" className="flex items-center justify-center gap-2 w-full py-3.5 bg-accent text-white font-bold rounded-lg transition-colors hover:bg-accent-dark">
