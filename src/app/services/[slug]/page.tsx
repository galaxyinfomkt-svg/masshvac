          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
            <ScrollReveal direction="left">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xl font-bold text-dark-900">Sound Familiar?</h2>
              </div>
            ...
            <ScrollReveal direction="right">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xl font-bold text-dark-900">We Fix It Right.</h2>
              </div>
              <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl shadow-[0_4px_20px_rgba(0,0,0,0.08)] p-8 md:p-12">
              {service.content.split("\n\n").map((paragraph, i) => {
                if (paragraph.startsWith("**") && paragraph.endsWith("**"))
                  return <h3 key={i} className="text-2xl font-bold text-dark-900 mt-8 mb-4">{paragraph.replace(/\*\*/g, "")}</h3>;
        ...
        <div className="mx-auto max-w-7xl px-4">
          <ScrollReveal className="text-center mb-10">
            <h2 className="text-2xl md:text-3xl font-bold text-dark-900">
              {service.shortName} Available In <span className="text-gold">Your City</span>
            </h2>
          </ScrollReveal>
          <div className="flex flex-wrap justify-center gap-2">
            {popularCities.map((city) => (
              <Link key={city.slug} href={`/cities/${city.slug}/${service.slug}`} className="px-4 py-2 bg-surface hover:bg-gold hover:text-black text-gray-600 text-sm font-medium rounded-full transition-all duration-200 hover:shadow-[0_4px_15px_rgba(0,0,0,0.1)]">
                {city.name}, MA
              </Link>
            ))}
            <Link href="/cities" className="px-4 py-2 bg-gold/10 text-gold text-sm font-bold rounded-full hover:bg-gold hover:text-black transition-all">
              All Cities →
            </Link>
          </div>
        </div>
